<!-- LOGO -->
<div align="center">
  <img src="path/to/your-logo.png" alt="Brian Hulela Logo" width="120">
</div>

<br>

<div align="center">
  <h1>Brian Hulela</h1>
</div>

<div align="center" style="max-width: 700px; margin: 0 auto; text-align: left;">
  <p>
    I am Brian Makhubele (Hulela), a Computer Science and Applied Mathematics graduate from the University of the Witwatersrand, with an Honours degree in Big Data Analytics.
  </p>

  <p>
    My work focuses on building practical systems across data, artificial intelligence, and software. I am interested in long-term digital infrastructure and the idea of Pax Africana through technology, where African-built systems create stability, ownership, and economic strength.
  </p>

  <h2>Focus Areas</h2>
  <ul>
    <li>Data Science and Machine Learning</li>
    <li>Artificial Intelligence Systems</li>
    <li>Cloud Computing (AWS, GCP)</li>
    <li>Software Engineering</li>
  </ul>

  <h2>Current Work</h2>
  <ul>
    <li>Developing production-ready systems using Python and cloud platforms</li>
    <li>Exploring machine learning applications in finance and decision systems</li>
    <li>Building and growing digital platforms through hulela.co.za</li>
  </ul>

  <h2>Skills</h2>

  <h3>Languages</h3>
  <ul>
    <li>Python</li>
    <li>JavaScript</li>
    <li>Java</li>
    <li>C++</li>
  </ul>

  <h3>Frameworks and Libraries</h3>
  <ul>
    <li>React</li>
    <li>TensorFlow</li>
    <li>PyTorch</li>
    <li>Pandas</li>
  </ul>

  <h3>Cloud</h3>
  <ul>
    <li>AWS</li>
    <li>Google Cloud</li>
  </ul>

  <h3>Databases</h3>
  <ul>
    <li>MySQL</li>
    <li>Firestore</li>
    <li>DynamoDB</li>
  </ul>

  <h2>Work and Writing</h2>
  <p>
    Website: <a href="https://hulela.co.za/" target="_blank">hulela.co.za</a><br>
    Medium: <a href="https://medium.com/@brianhulela" target="_blank">@brianhulela</a><br>
    Substack: <a href="https://substack.com/@brianhulela" target="_blank">@brianhulela</a>
  </p>

  <h2>Additional Interests</h2>
  <p>
    I work on digital art and 3D animation as a way to explore visual thinking and creative systems.
  </p>

  <hr>

  <p>
    If you are building something meaningful, I am open to collaboration.
  </p>
</div>
